...
const RichTextEditor: React.FC<{ value: string; onChange: (value: string) => void; }> = ({ value, onChange }) => {
    ...
    return (
        <div className="border border-border-light dark:border-border-dark rounded-lg flex flex-col h-full bg-transparent">
            <div className="flex items-center gap-1 p-2 border-b border-border-light dark:border-border-dark bg-surface-muted-light dark:bg-surface-muted-dark rounded-t-lg flex-shrink-0">
            ...
            <div
                ...
                className="p-4 flex-grow overflow-y-auto bg-transparent focus:outline-none rounded-b-lg prose dark:prose-invert max-w-none prose-sm"
            />
        </div>
    );
};
