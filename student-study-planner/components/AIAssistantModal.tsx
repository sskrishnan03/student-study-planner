...
const AIAssistantModal: React.FC<{ isOpen: boolean; onClose: () => void; noteContent: string }> = ({ isOpen, onClose, noteContent }) => {
    ...
    // Memoize the AI instance to avoid re-creation on re-renders.
    ...
    const handleAction = async (action: AIAction) => {
        ...
        const prompts = {
            summarize: `Summarize the following study note concisely. Focus on the key points and main ideas. Here is the note:\n\n---\n\n${noteContent}`,
            quiz: `Based on the following study note, generate a short, 3-question multiple-choice quiz to test understanding. Provide the questions, options, and then list the correct answers at the very end. Here is the note:\n\n---\n\n${noteContent}`,
            explain: `Explain the core concepts from the following study note in a simple and easy-to-understand way, as if you were explaining it to a beginner. Here is the note:\n\n---\n\n${noteContent}`,
        };
