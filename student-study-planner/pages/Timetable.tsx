...
import { PlusIcon, ClockIcon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/outline';
...
const EventForm: React.FC<{ 
    ...
    const [formData, setFormData] = useState({
        title: event?.title || '',
        day: event?.day ?? 1, // Default to Monday, use ?? to handle day 0 (Sunday)
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="space-y-4">
                ...
                <select name="day" value={formData.day} onChange={handleChange} className={inputStyles}>
...
const Timetable: React.FC = () => {
  ...
  const getEventPosition = (event: TimetableEvent) => {
      ...
      const top = (start / (15 * 60)) * 100;
      const height = ((end - start) / (15 * 60)) * 100;
  ...
  const getHeaderTitle = () => {
    switch(currentView) {
        case 'month': return currentDate.toLocaleDateString(undefined, { month: 'long', year: 'numeric' });
        case 'day': return currentDate.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
        case 'week':
        default: return 'Weekly Timetable';
    }
  };
  ...
  const renderWeeklyView = () => (
    <Card>
        <div className="grid grid-cols-[auto_repeat(7,1fr)]">
            ...
            {daysOfWeek.map(day => <div key={day} className="text-center font-semibold pb-2 border-b-2 border-border-light dark:border-border-dark text-text-primary-light dark:text-text-primary-dark">{day}</div>)}
            ...
            <div className="col-start-2 col-span-7 grid grid-cols-7 relative">
                {daysOfWeek.map((_, dayIndex) => (
                    <div key={dayIndex} className="border-l border-border-muted-light dark:border-border-muted-dark">
                ...
                {events.map(event => (
                    <div key={event.id} className={`absolute text-white p-2 rounded-lg overflow-hidden cursor-pointer shadow-lg hover:shadow-xl transition-shadow border border-white/20`} style={{...getEventPosition(event), left: `${(event.day / 7) * 100}%`, width: `${(1/7) * 100}%`, backgroundColor: event.color}} onClick={() => handleEdit(event)}>
            ...
            <div className="grid grid-cols-7 grid-rows-5">
                {calendarDays.map((day, index) => (
                    <div key={index} className="h-32 border-b border-r border-border-muted-light dark:border-border-muted-dark p-2 overflow-hidden flex flex-col cursor-pointer hover:bg-surface-muted-light dark:hover:bg-surface-muted-dark" onClick={() => day && (setCurrentDate(day), setCurrentView('day'))}>
                        {day && (
                            <>
                                <span className={`font-semibold text-sm ${isSameDay(day, today) ? 'bg-primary text-white rounded-full h-6 w-6 flex items-center justify-center' : ''}`}>{day.getDate()}</span>
                                <div className="mt-1 space-y-1 overflow-y-auto">
                                {events.filter(e => e.day === day.getDay()).sort((a,b) => a.startTime.localeCompare(b.startTime)).map(event => (
  ...
  const renderDailyView = () => {
    ...
    const dayEvents = events.filter(e => e.day === dayOfWeek).sort((a,b) => a.startTime.localeCompare(b.startTime));
    return (
        <Card>
